...
class ConsultaActivo
{
  ...
  public function obtenerActivo($n_etiqueta){
    try {

      $this->conexion = $this->conexionBD->abrirConexion();

      //seguridad
      $stmt =  $this->conexion->prepare("CALL tb_activo_obtener_by_n_etiqueta(?)");
      $stmt->bindParam(1, $n_etiqueta);
      $stmt->execute();
      $fila = $stmt->fetchObject();
      $stmt = NULL;
      ...
      if($fila){
        ...
        $this->json["activo"] = null;
        $this->json["success"] = false;
        $this->json["mensaje"] = "Busqueda no exitosa!";
      }
