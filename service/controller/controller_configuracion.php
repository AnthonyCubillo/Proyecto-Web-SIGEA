...
if (isset($_GET["consulta"])) {
  ...
  switch ($_GET["consulta"]) {
    ...
    case 2: //editar
      ...
      if (
        isset($data["id_configuracion"]) && !empty($data["id_configuracion"])
        ...
        $configuracion = new  configuracion($data["id_configuracion"], $data["color"], $data["font_family"], $data["font_size"], 0);
        $consultaConfiguracion->actualizarConfiguracion($configuracion);
      } else {
        // respuesta si no envia parametros
    ...
    case 3: //insertar
      ...
      if (isset($data["id_usuario"]) && !empty($data["id_usuario"])) {
        ...
        $consultaConfiguracion = new ConsultaConfiguracion();
        $consultaConfiguracion->insertarConfiguracion($data["id_usuario"]);
      } else {
        // respuesta si no envia parametros
        $json["id_usuario"] = $data["id_usuario"];
        $json["mensaje"] = "Error una o varias entradas estan vacias!";
