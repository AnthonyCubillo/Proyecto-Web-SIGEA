...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 04-05-2023 a las 22:35:24
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_activo_obtener_by_n_etiqueta` (IN `_n_etiqueta` VARCHAR(50))   BEGIN
SELECT tb_activo.n_etiqueta,tb_activo.marca,tb_activo.modelo,tb_activo.serie,tb_activo.descripcion,tb_activo.id_ubicacion,tb_activo.valor_libro,tb_activo.condicion,tb_activo.clase_activo,tb_activo.id_funcionario, tb_funcionario.nombre_funcionario, tb_ubicacion.nombre_ubicacion,tb_funcionario.dni_funcionario  FROM `tb_activo` INNER JOIN tb_funcionario ON tb_activo.id_funcionario = tb_funcionario.id_funcionario INNER JOIN tb_ubicacion ON tb_activo.id_ubicacion = tb_ubicacion.id_ubicacion WHERE tb_activo.estado = 1 AND tb_activo.n_etiqueta= _n_etiqueta;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_alerta_actualizar_alerta` (IN `alerta_n_etiqueta` VARCHAR(50), IN `alerta_marca` VARCHAR(50), IN `alerta_modelo` VARCHAR(50), IN `alerta_serie` VARCHAR(50), IN `alerta_activo_descripcion` VARCHAR(250), IN `alerta_valor_libro` VARCHAR(50), IN `alerta_condicion` VARCHAR(50), IN `alerta_clase_activo` VARCHAR(50), IN `alerta_id_funcionario` INT)   BEGIN
    UPDATE tb_alerta
    SET 
        tb_alerta_marca = alerta_marca,
        tb_alerta_modelo = alerta_modelo,
        tb_alerta_serie = alerta_serie,
        tb_alerta_activo_descripcion = alerta_activo_descripcion,
        tb_alerta_valor_libro = alerta_valor_libro,
        tb_alerta_condicion = alerta_condicion,
        tb_alerta_clase_activo = alerta_clase_activo,
        tb_alerta_id_funcionario = alerta_id_funcionario
    WHERE tb_alerta_n_etiqueta = alerta_n_etiqueta;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_alerta_eliminar_por_id` (IN `alerta_n_etiqueta` INT)   BEGIN
    DELETE FROM tb_alerta WHERE tb_alerta_n_etiqueta = alerta_n_etiqueta;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_alerta_ingresar_alerta` (IN `alerta_n_etiqueta` VARCHAR(50), IN `alerta_marca` VARCHAR(50), IN `alerta_modelo` VARCHAR(50), IN `alerta_serie` VARCHAR(50), IN `alerta_activo_descripcion` VARCHAR(250), IN `alerta_ubicacion` INT, IN `alerta_valor_libro` VARCHAR(50), IN `alerta_condicion` VARCHAR(50), IN `alerta_clase_activo` VARCHAR(50), IN `alerta_id_funcionario` INT, IN `alerta_fecha` DATETIME(3), IN `alerta_tipo_alerta` INT)   BEGIN
    INSERT INTO tb_alerta (
        tb_alerta_n_etiqueta, 
        tb_alerta_marca, 
        tb_alerta_modelo, 
        tb_alerta_serie,
        tb_alerta_activo_descripcion, 
        tb_alerta_ubicacion, 
        tb_alerta_valor_libro,
        tb_alerta_condicion,
        tb_alerta_clase_activo,
        tb_alerta_id_funcionario,
        tb_alerta_fecha, 
        tb_alerta_tipo_alerta
    )
    VALUES (
        alerta_n_etiqueta, 
        alerta_marca, 
        alerta_modelo, 
        alerta_serie, 
        alerta_activo_descripcion,
        alerta_ubicacion, 
        alerta_valor_libro, 
        alerta_condicion,
        alerta_clase_activo, 
        alerta_id_funcionario, 
        alerta_fecha, 
        alerta_tipo_alerta
    );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_alerta_mostrar_alertas` ()   BEGIN
    SELECT fun.nombre_funcionario as tb_alerta_nombre_funcionario,
    fun.dni_funcionario as tb_alerta_dni_funcionario,
    tb_alerta_id, tb_alerta_n_etiqueta, tb_alerta_marca, tb_alerta_modelo, tb_alerta_serie,
           tb_alerta_activo_descripcion, tb_alerta_ubicacion,tb_alerta_condicion,
           tb_alerta_clase_activo, tb_alerta_valor_libro, tb_alerta_id_funcionario, tb_alerta_fecha,
           tb_alerta_tipo_alerta
    FROM tb_alerta
    INNER JOIN tb_funcionario as fun on fun.id_funcionario = tb_alerta.tb_alerta_id_funcionario
    ORDER BY tb_alerta_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_configuracion_actualizar` (IN `_color` VARCHAR(30), IN `_font_family` VARCHAR(30), IN `_font_size` VARCHAR(30), IN `_id_configuracion` INT(11))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_configuracion_eliminar` (IN `_id_usuario` INT(11))   BEGIN
    DELETE FROM tb_configuracion WHERE _id_usuario = id_usuario;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_configuracion_insertar` (IN `_id_usuario` VARCHAR(50))   BEGIN 
INSERT INTO `tb_configuracion`(`color`, `font_family`, `font_size`,`id_usuario`)
VALUES ('#F8F9F9', 'Yantramanav', '16px', _id_usuario);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tb_configuracion_obtener` (IN `_id_usuario` INT(11))   BEGIN
...
('N00114281', 'HEWLETT PACKARD', 'DC5750', 'MXJ80807XG', 'COMPUTADORA DE ESCRITORIO', 1, '0,00', 'En desuso', 'Computadoras', 1, 0),
('N00114283', 'HEWLETT PACKARD', 'DC5750', 'MXJ80807XG', 'COMPUTADORA DE ESCRITORIO', 1, '0,00', 'En desuso', 'Computadoras', 1, 0),
('N00114284', 'HEWLETT PACKARD', 'DC5750', 'MXJ80807XG', 'COMPUTADORA DE ESCRITORIO', 1, '0,00', 'En desuso', 'Computadoras', 1, 0),
('N00114285', 'HEWLETT PACKARD', 'DC5750', 'MXJ80807XG', 'COMPUTADORA DE ESCRITORIO', 1, '0,00', 'En desuso', 'Computadoras', 1, 1),
('N00116179', 'SONY', 'CFD-RS60CP', 'S01-1085335-B', 'RADIOGRABADORA', 1, '0,00', 'En uso', 'Equipos de audio y video', 1, 1),
('N00117441', 'EUROMOBILIA', 'BS', 'NO TIENE', 'ESTACION DE TRABAJO SECRETARIAL', 1, '0,00', 'En uso', 'Mesas y escritorios', 1, 1),
('N00117750', 'EUROMOBILIA', 'D', 'NO TIENE', 'BIBLIOTECA BAJA', 1, '0,00', 'En uso', 'Archivadores, bibliotecas y armarios', 1, 0),
('N00123877', 'H.P.', '3005', 'MXLO1207HQ', 'COMPUTADORA DE ESCRITORIO', 1, '0,00', 'En uso', 'Computadoras', 1, 1),
('N00127613', 'HP ', 'ProBook 430 G7 ', '5CD1119HXP', 'COMPUTADORA PORTATIL (ESTANDAR I)', 5, '343.666,72', 'En uso', 'Computadoras', 1, 1),
('N001276130', 'HP ', 'ProBook 430 G7 ', '5CD1119HWK', 'COMPUTADORA PORTATIL (ESTANDAR I)', 1, '343.666,72', 'En uso', 'Computadoras', 1, 1),
('N00135505', 'CISCO', '6921', 'SPUC17220U5A', 'TELEFONO', 1, '19.811,32', 'En uso', 'Equipos de telefonía', 1, 1),
('N00147795', 'HP', 'PRODESK 600 G2 SFF', 'MXL7031HTN', 'COMPUTADORA DE ESCRITORIO', 1, '0,00', 'En uso', 'Computadoras', 1, 1),
('N00169760', 'FANTINI', 'N/A', '1', 'MESA RECTANGULAR PARA SALA DE REUNIONES TIPO C1M (PARA FUERA DE LA GRAN AREA METROPOLITANA)', 1, '225.393,83', 'En uso', 'Mesas y escritorios', 1, 1),
('N00169761', 'FANTINI', 'N/A', '1', 'MESA RECTANGULAR PARA SALA DE REUNIONES TIPO C2M (PARA FUERA DE LA GRAN AREA METROPOLITANA)', 1, '245.366,79', 'En uso', 'Mesas y escritorios', 1, 1),
('N00187610', 'N00127613', 'N00127613', 'N00127613', 'N00127613', 1, 'N00127613', 'N00127613', 'N00127613', 0, 1),
...
--
-- Estructura de tabla para la tabla `tb_alerta`
--

CREATE TABLE `tb_alerta` (
  `tb_alerta_id` int(11) NOT NULL,
  `tb_alerta_n_etiqueta` varchar(50) NOT NULL,
  `tb_alerta_marca` varchar(50) NOT NULL,
  `tb_alerta_modelo` varchar(50) NOT NULL,
  `tb_alerta_serie` varchar(50) NOT NULL,
  `tb_alerta_activo_descripcion` varchar(250) NOT NULL,
  `tb_alerta_ubicacion` int(11) DEFAULT NULL,
  `tb_alerta_valor_libro` varchar(50) NOT NULL,
  `tb_alerta_condicion` varchar(50) NOT NULL,
  `tb_alerta_clase_activo` varchar(50) NOT NULL,
  `tb_alerta_id_funcionario` int(11) NOT NULL,
  `tb_alerta_fecha` date NOT NULL,
  `tb_alerta_tipo_alerta` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;

--
-- Volcado de datos para la tabla `tb_alerta`
--

INSERT INTO `tb_alerta` (`tb_alerta_id`, `tb_alerta_n_etiqueta`, `tb_alerta_marca`, `tb_alerta_modelo`, `tb_alerta_serie`, `tb_alerta_activo_descripcion`, `tb_alerta_ubicacion`, `tb_alerta_valor_libro`, `tb_alerta_condicion`, `tb_alerta_clase_activo`, `tb_alerta_id_funcionario`, `tb_alerta_fecha`, `tb_alerta_tipo_alerta`) VALUES
(1, 'N00114280', 'marca', 'modelo', 'serie', 'descripcion', 1, '50000', 'condicion uso', 'clase activo', 1, '2023-04-19', 3),
(16, '66666', 'marca', 'modelo', 'serie', 'descripcion', 1, '2626', 'condicion uso', 'clase activo', 1, '2023-04-29', 1),
(18, '9788', 'Bonaldaaaaaaa', 'A4D0', 'N/A', 'Escritorio gris, con cajones', 1, '300000', 'En uso', 'Mesa y escritorios', 1, '2023-04-30', 1);
...
(19, '#990000', 'Georgia', '14px', '1'),
(27, '#F8F9F9', 'Yantramanav', '16px', '4');
...
(6, 'Aula 68', 'Infraestructura para impartir clases', 1);
...
(1, '123456789', 'José María Figueres Olsen', 'wilfredobarqueroherrera@gmail.com', '$2y$10$VRD1gjLwro6WR6XzcapfeOacqbiAXb1L7TZkY1qmvvuRM2rI/PQcS', 1),
(4, '702050830', 'Wilfredo Barquero Herrera', 'wilfredobarqueroherrera@gmail.com', '$2y$10$BN1KhwEqX1pKhNvFvAAE7O.tZbMUWIhNXVuG3dSqEKCJlMO0k0.fS', 1);
...
--
-- Indices de la tabla `tb_alerta`
--
ALTER TABLE `tb_alerta`
  ADD PRIMARY KEY (`tb_alerta_id`);
...
--
-- AUTO_INCREMENT de la tabla `tb_alerta`
--
ALTER TABLE `tb_alerta`
  MODIFY `tb_alerta_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;
